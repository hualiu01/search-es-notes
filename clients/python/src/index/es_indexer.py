...
class EsIndexer(EsClient):
    ...
    @function_timer
    def new_index(self, index_name: str, force_recreate : bool =False, index_settings :dict=None):
        """
        ...
        if self.get_index_info(index_name) is not None:
            if force_recreate:
                logger.warning(f"WARN: index {index_name} already exists. Force recreating by deleting and creating again now...")
                self.delete_index(index_name)
            else:
                logger.warning(f"WARN: index {index_name} already exists. Skipped recreating.")
                return
